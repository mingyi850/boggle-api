...
class GamesController < ApplicationController
  ...
  def create
    ...
    if game.save
      render json: game.as_json(only: %i[id token duration board]),
             status: :created
    else
      render json: {message: 'Error', data: game.errors},
             status: :unprocessable_entity
    end
  rescue GamesCreator::GameCreationError => e
    if e.instance_of?(GamesCreator::InvalidBoardError)
      render json: {message: 'Error: Invalid Board Passed', data: params},
             status: :bad_request
    end
  end
  ...
  def handle_game_error(error)
    if error.instance_of?(GamesManager::InvalidTokenError)
      ...
      render json: { message: 'Invalid Word' }, status: :not_acceptable
    end
  end
